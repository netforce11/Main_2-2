...
class RightPanelMixin:
    ...
    def _build_leg_right(self) -> QWidget:
        ...
        # ════════════════════════════════════════════════════
        # 거래소 선택 체크박스: SMART / CBOE
        # ════════════════════════════════════════════════════
        _exch_sep = QLabel("│")
        _exch_sep.setStyleSheet(f"color:{_t.get('group_border','#333')};font-size:16px;border:none;")
        _exch_sep.setFixedWidth(12)
        _exch_sep.setAlignment(Qt.AlignCenter)
        grp_a.addWidget(_exch_sep)

        _exch_lbl = QLabel("거래소:")
        _exch_lbl.setStyleSheet(f"color:{_t.get('label_fg','#aaa')};font-size:11px;border:none;")
        grp_a.addWidget(_exch_lbl)

        self.chk_exch_smart = QCheckBox("SMART")
        self.chk_exch_smart.setChecked(True)
        self.chk_exch_smart.setStyleSheet(
            "QCheckBox{color:#00ff88;font-size:11px;spacing:3px;}"
            "QCheckBox::indicator{width:13px;height:13px;}"
            "QCheckBox::indicator:checked{background:#00ff88;border:1px solid #00ff88;}"
            "QCheckBox::indicator:unchecked{background:#111;border:1px solid #555;}")
        grp_a.addWidget(self.chk_exch_smart)

        self.chk_exch_cboe = QCheckBox("CBOE")
        self.chk_exch_cboe.setChecked(False)
        self.chk_exch_cboe.setStyleSheet(
            "QCheckBox{color:#ffaa44;font-size:11px;spacing:3px;}"
            "QCheckBox::indicator{width:13px;height:13px;}"
            "QCheckBox::indicator:checked{background:#ffaa44;border:1px solid #ffaa44;}"
            "QCheckBox::indicator:unchecked{background:#111;border:1px solid #555;}")
        grp_a.addWidget(self.chk_exch_cboe)

        # 상호 배타: 한쪽 체크 시 반대쪽 해제
        def _on_smart_toggled(checked, _self=self):
            if checked:
                _self.chk_exch_cboe.setChecked(False)
            elif not _self.chk_exch_cboe.isChecked():
                _self.chk_exch_smart.setChecked(True)   # 둘 다 해제 방지

        def _on_cboe_toggled(checked, _self=self):
            if checked:
                _self.chk_exch_smart.setChecked(False)
            elif not _self.chk_exch_smart.isChecked():
                _self.chk_exch_cboe.setChecked(True)    # 둘 다 해제 방지

        self.chk_exch_smart.toggled.connect(_on_smart_toggled)
        self.chk_exch_cboe.toggled.connect(_on_cboe_toggled)
