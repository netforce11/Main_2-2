...
────────────────────────────────────────────────────────────
사용법:
  python apply_patch_v2.py [프로젝트_루트_경로]

  예) python apply_patch_v2.py
      python apply_patch_v2.py /home/netforce/tab_chart
...
def find_project_root(hint: str = None) -> Path:
    """
    프로젝트 루트(tab_chart_libs 가 있는 상위 폴더 또는 직접 경로) 탐색.
    ...
    # 스크립트 위치 기준 상위 순서로 탐색
    candidates += [
        SCRIPT_DIR,                          # tab_chart_libs 자체
        SCRIPT_DIR.parent,                   # 상위 폴더
        Path.cwd(),
        Path.cwd() / "tab_chart_libs",
    ]

    for p in candidates:
        if (p / "chart_build_side_bottom.py").exists():
            return p
        # 하위에 tab_chart_libs 폴더가 있는 경우
        sub = p / "tab_chart_libs"
        if sub.is_dir() and (sub / "chart_build_side_bottom.py").exists():
            return sub
