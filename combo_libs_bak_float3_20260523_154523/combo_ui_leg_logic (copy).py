...
────────────────────────────────────────────────────
v3.1 버그 수정:
  BUG-2  _rebuild_legs: blockSignals(True/False)로 itemChanged 폭발 방지
  BUG-4  _maybe_show_setup_guide: suppress 사전 체크 후 조건부 객체 생성
  BUG-11 _maybe_show_setup_guide: 역방향(strat in key) 매칭 제거
────────────────────────────────────────────────────
"""
...
def _maybe_show_setup_guide(self, strat: str):
    ...
    BUG-4  수정: suppress 여부를 객체 생성 전에 먼저 확인.
    BUG-11 수정: 'key in strat' 단방향 매칭만 사용 (역방향 'strat in key' 제거).
    """
    from combo_constants import STRATEGY_SETUP_GUIDE
    from combo_strategy_guide_popup import _SUPPRESS_SET

    # 정확 매칭 우선, 없으면 단방향(key ⊂ strat) 부분 매칭
    guide = STRATEGY_SETUP_GUIDE.get(strat)
    if guide is None:
        for key in STRATEGY_SETUP_GUIDE:
            if key in strat:          # BUG-11: strat in key 제거
                guide = STRATEGY_SETUP_GUIDE[key]
                break
    if guide is None:
        return

    # BUG-4: suppress면 객체 생성 자체를 건너뜀
    if guide.get("title", "") in _SUPPRESS_SET:
        return
...
def _rebuild_legs(self, legs: list):
    """
    레그 테이블 재구성.
    BUG-2 수정: blockSignals(True) → setItem 루프 → blockSignals(False)
                아이언 콘도르 기준 28번이던 itemChanged가 0번으로 줄어듦.
    """
    ...
    # BUG-2: 삽입 구간 시그널 차단 (예외 발생 시에도 finally로 반드시 해제)
    self.tbl_legs.blockSignals(True)
    try:
        self.tbl_legs.setRowCount(0)
        for i, leg in enumerate(legs):
            _insert_leg_row(self, leg, i)
    finally:
        self.tbl_legs.blockSignals(False)
    ...
    # 재구성 완료 후 1회 일괄 갱신 (blockSignals 해제 이후)
    try:
...
def _insert_leg_row(self, leg: dict, row_idx: int):
    """레그 1행을 tbl_legs에 삽입. (blockSignals 블록 내부에서 호출됨)"""
