...
# ── [ADD] 급변 감시 텔레그램 헬퍼 ────────────────────────────
def _spike_tg(ev) -> None:
    """SpikeEvent → 텔레그램 전송 (별도 스레드, 실패 시 무시)."""
    try:
        arrow = "▼" if ev.curr < ev.prev else "▲"
        msg = (
            f"⚡ 급변 감시 알림\n"
            f"{ev.time_str}  {ev.cp} {int(ev.strike)}\n"
            f"${ev.prev:.2f} → ${ev.curr:.2f} "
            f"({arrow}{ev.tick_delta}틱 / {ev.pct_change:+.1f}%)\n"
            f"트리거: {ev.trigger}"
        )
        from telegram_bot.tg_client import TelegramClient
        import threading
        threading.Thread(
            target=TelegramClient.get().send,
            args=("order_confirm", msg),
            daemon=True).start()
    except Exception as e:
        print(f"[SpikeTab] TG 실패: {e}")
