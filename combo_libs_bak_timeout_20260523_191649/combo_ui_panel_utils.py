...
def _enrich_strategy_name(pos: dict) -> str:
    ...
    try:
        strikes = "/".join(
            ...
            for l in sorted(legs, key=lambda l: float(l.get("strike", 0)), reverse=True)
            if l.get("strike")
        )
        base = pos.get("strategy", "")
        # 이미 행사가 패턴(숫자/숫자)이 있으면 추가 안 함
        already = bool(re.search(r'\d+/\d+', base))
        if strikes and not already:
            if "(" in base:
                idx = base.index("(")
