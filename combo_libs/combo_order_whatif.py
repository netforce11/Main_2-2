...
def _send_whatif_order(self, legs: list, strat: str, cost_str: str, on_done=None):
    ...
    # ── 구버전 _request_margin_then_order 가 패치해둔 콜백이
    #    잔류해 있으면 bridge.acct_value 가 emit 되지 않으므로 원복 ──
    if hasattr(ib, '_orig_acct_summary'):
        try:
    ...
    def _send_legs():
        ...
        # ── 레그 전송 완료 후 타임아웃 카운트 시작 ──────────────
        # (reqAccountSummary 직후가 아닌 실제 whatIf 전송 직후부터 측정)
        def _check_timeout(sid):
            if sid != self._whatif_session:
                return
            if not self._whatif_buf:
                self._log("⚠ whatIf 타임아웃 — IB 응답 없음 (레그 전송 후 10초 경과)")
                self._whatif_session = -1
