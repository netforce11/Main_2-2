"""
sleep_order_config.py — 수면 예약 주문 설정 저장소  v2.0
════════════════════════════════════════════════════════════════
...
설정 항목:
  [섹션 A] 예약 주문
    ...
    spread_width        스프레드 폭 ($)
    target_price_1      목표 net price 1  (이하 조건 — <= 비교)
    target_price_2      목표 net price 2  (이하 조건 — <= 비교)
    max_budget          최대 투자 금액 ($)
    schedule_enabled    예약 감시 ON/OFF
    aggressive_entry    Aggressive Entry ON/OFF (슬리피지 보정)
    aggressive_ticks    보정 틱 수 (1~3)
  ...
  [섹션 B] 급락 캐치
    ...
    cancel_cooldown_sec 취소 후 재주문 차단 시간 (초)
════════════════════════════════════════════════════════════════
"""
...
_DEFAULTS: dict = {
    ...
    "roi_min":            0,     # 최소 수익률 (%, 0 = 제한 없음)
    "roi_max":            1200,  # 최대 수익률 (%)

    # ── Aggressive Entry ──────────────────────────────────────
    # True = 현재가보다 N틱 위로 주문 → 체결률 향상
    # 틱 크기: net < $3.00 → $0.05 / net >= $3.00 → $0.10
    "aggressive_entry":   True,    # 기본 ON
    "aggressive_ticks":   1,       # 보정 틱 수 (1~3 권장)

    # ── 취소 후 쿨다운 ────────────────────────────────────────
    # 수동 취소 후 자동 재주문을 차단하는 대기 시간 (초)
    "cancel_cooldown_sec": 120,    # 기본 2분
