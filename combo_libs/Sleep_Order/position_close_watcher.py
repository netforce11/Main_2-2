...
class PositionCloseWatcher(QObject):
    ...
    def on_order_placed(self, oid: int) -> None:
        """[FIX-ACTIVE-OID] _do_send_body placeOrder 성공 직후 호출.
        ORDERED 상태에서 active_oid 가 None 인 슬롯에 OID 확정 등록.
        비동기 conId 조회 타이밍 문제 완전 차단.
        """
        for slot in self._slots:
            if slot.state == _SlotState.ORDERED and slot.active_oid is None:
                slot.active_oid = oid
                print(f"[ClosWatcher] on_order_placed: 슬롯{slot.idx+1} active_oid={oid} 확정")
                break
    ...
    def _send_order(self, slot: _SlotState, is_correction: bool) -> None:
        ...
        try:
            ...
            # 발사 직후 ref에 저장된 최신 chaser OID 획득
    ...
    def _send_market(self, slot: _SlotState) -> None:
        ...
        if bid_price and bid_price > 0:
            ...
            QTimer.singleShot(5000, lambda s=slot: self._last_resort_market(s))
        else:
            print(f"[ClosWatcher] 슬롯{slot.idx+1} Bid 조회 실패 → 즉시 시장가")
            self._last_resort_market(slot)

    def _last_resort_market(self, slot: _SlotState) -> None:
        """[FIX-SLIPPAGE+LOOP] 최후 시장가 — 1회만 발사."""
        if slot.state != _SlotState.ORDERED:
            return
        if not self._pos_exists(slot):
            return
