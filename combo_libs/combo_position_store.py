...
def restore_on_reconnect(self) -> None:
    ...
    [v4.0] 올바른 복원 순서:
      1단계) IB reqPositions() 발사 (서버 잔고 먼저 확인)
      2단계) positionEnd 수신
             ├─ IB 잔고 있음 → reqExecutions로 OID 매칭 → 파일과 병합 → 표시
             └─ IB 잔고 없음 → reqExecutions로 청산 여부 확인
                  ├─ 청산 확인 → 파일 제거
                  └─ 청산 없음 → 파일 그대로 표시 (보유 중)
      3단계) 30초 타임아웃 → 파일 즉시 표시 (IB 무응답 폴백)
    """
    # [FIX-PANEL] synthetic_panel 다단계 탐색
    panel = getattr(self, 'synthetic_panel', None)
    if panel is None:
        # mw 경유 탐색
        mw = getattr(self, 'mw', None)
        for attr in ('combo_tab', 'tab_combo', 'tab_strategy', 'strategy_grid'):
            t = getattr(mw, attr, None) if mw else None
            if t and hasattr(t, 'synthetic_panel'):
                panel = t.synthetic_panel; break
    if panel is None:
        try:
    ...
    ib = (getattr(getattr(self, 'mw', None), 'ib', None) or
          getattr(self, 'ib', None))
    if panel is None or ib is None:
        print(f"[PosStore] restore 실패: panel={panel} ib={ib}")
        return

    # [FIX-2] 복원 진행 중 중복 차단 — _on_pos_reconnect_hook 에서 이미 처리됨
    ...
    # ── 폴백: 파일 표시 함수 (타임아웃 or IB 잔고 없을 때 사용) ─
    def _show_file_positions(reason="파일"):
        shown = skipped = 0
        panel.clear_positions()
    ...
    def _restore_done(show_file=False):
        """콜백 원복 + 플래그 해제 공통 처리."""
        if _restored[0]:
            return
        _restored[0] = True
        ...
        if show_file:
            QTimer.singleShot(0, lambda: _show_file_positions("타임아웃 폴백"))

    def _on_pos(account, contract, pos_qty, avg_cost):
        # [FIX-XSP-RESTORE] qty=0 또는 극소 수량 → 만기 현금결제 잔재 → 무시
        if abs(pos_qty) < 0.001:
            return
        sec = getattr(contract, 'secType', '')
        if sec not in ('OPT', 'FOP', 'BAG'):
            return
        # [FIX-XSP-RESTORE] XSP 만기 포지션 필터 — expiry 가 오늘 이전이면 무시
        try:
            from datetime import date
            expiry_str = str(getattr(contract, 'lastTradeDateOrContractMonth', '') or '')
            if len(expiry_str) >= 8:
                exp_date = date(int(expiry_str[:4]),
                                int(expiry_str[4:6]),
                                int(expiry_str[6:8]))
                if exp_date < date.today():
                    sym = getattr(contract, 'symbol', '')
                    print(f"[PosStore] 만기 포지션 무시: {sym} {expiry_str} qty={pos_qty}")
                    return
        except Exception:
            pass
    ...
    def _on_pos_end():
        ...
        if not ib_buf:
            ...
            # IB 잔고 없음 → executions 확인 후 파일 표시
            _check_executions_and_clean(
                self, panel, saved_by_oid,
                on_done=lambda: _show_file_positions("청산확인 후 파일"))
            return
        ...
        # ── [v4.0] 파일 포지션 먼저 패널에 표시 ─────────────
        _show_file_positions("IB서버 확인 후 파일")

        # ── qty/entry 보정: IB 서버값으로 덮어쓰기 ───────────
        updated = 0
        for oid, saved in saved_by_oid.items():
            ...
            if matched and hasattr(panel, '_positions'):
                for p in panel._positions:
                    if p.get('oid') == oid:
                        # [FIX-ENTRY] qty만 보정 — entry는 BAG 체결가(파일값) 유지
                        # IB avg_cost는 개별 레그 단위라 콤보 entry와 단위 불일치
                        p['qty'] = matched[0]["qty"]
                        updated += 1
                        self._log(
                            f"✏ qty 보정: OID={oid} qty={matched[0]['qty']}")

        # ── IB 서버에만 있는 포지션 → 패널/파일 추가 ───────
        added = 0
        for ib_pos in ib_buf:
            sig = _ib_sig_simple(ib_pos)
    ...
    # [v4.0] 타임아웃: positionEnd 미수신 시 파일 표시로 폴백 (30초)
    def _timeout():
        if _restored[0]:
            return
...
def _check_executions_and_clean(self, panel, saved_by_oid: dict,
                                on_done=None) -> None:
    """
