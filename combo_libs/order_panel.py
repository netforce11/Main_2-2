...
class OrderPanelMixin:
    ...
    def _emergency_sell_positions(self):
        ...
        ib.tickPrice 직접 교체 제거 → price_tick_sig 구독 방식으로 전환.
        이제 긴급매도 실행 중에도 다른 모듈(Chaser, 스나이퍼 등)의
        시세 수신이 방해받지 않는다.

        가격 결정 우선순위 (포지션당):
          1) 실시간 Bid  — _emrg_bid_cache[sym]
        ...
        # Bid / Last 캐시 (reqId → {sym, bid, last})
        self._emrg_bid_cache  = {}   # sym → bid price
        self._emrg_last_cache = {}   # sym → last price
        self._emrg_reqid_to_sym = {} # reqId → sym

        _pos_buf = []   # [(contract, qty, avg_cost)]

        # [L-A] price_tick_sig 임포트
        try:
        ...
        # ── 시세 수신 핸들러 (Bid=1, Last=4) ─────────────────────
        def _emrg_tick(req_id: int, tick_type: int, price: float) -> None:
            """긴급매도 전용 Bid/Last 캐싱. reqId 범위(8850~)만 처리."""
            if price <= 0:
                return
        ...
        # 관심 reqId 집합 (포지션 수신 후 채워짐, 일단 전체 구독)
        if _use_router:
            _sub(_emrg_tick)   # 포지션 reqId는 나중에 알므로 전체 수신
        ...
        def _restore():
            # [L-A] ib.tickPrice 복원 불필요 — price_tick_sig 구독만 해제
            if _use_router:
                try:
                    _unsub(_emrg_tick)
                except Exception:
