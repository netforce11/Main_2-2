...
class SyntheticStatusPanel(_SyntheticPanelBuildMixin, QWidget):
    ...
    def _on_close_mkt(self):
        ...
        _log_dir = Path.home() / "trading_logs" / "close_debug"
        _log_dir.mkdir(parents=True, exist_ok=True)
        _log_file = _log_dir / f"{datetime.now().strftime('%Y%m%d')}_청산.log"
        def _dlog(msg):
            ts = datetime.now().strftime("%H:%M:%S")
            with open(_log_file, "a", encoding="utf-8") as f:
                f.write(f"[{ts}] {msg}\n")
            print(f"[CLOSE_DEBUG] {msg}")
        row = getattr(self, '_selected_pos_row', -1)
        _dlog(f"_on_close_mkt called row={row} callback={self._close_pos_callback}")
        if row < 0 or row >= len(self._positions):
            _dlog(f"row 범위 오류 len={len(self._positions)}")
            return
        pos = self._positions[row]
        _dlog(f"pos status={pos.get('status')} oid={pos.get('oid')} strategy={pos.get('strategy')}")
        if callable(self._close_pos_callback):
            _dlog("콜백 호출 → _on_close_position_order")
            self._close_pos_callback(pos, lmt_price=None)
        else:
            _dlog("ERROR: _close_pos_callback 이 None — 콜백 미연결")
    ...
    def _refresh_pos_table(self):
        ...
        for r, pos in enumerate(self._positions):
            ...
            # 칼럼 9: 청산 예약 + 수동 삭제 버튼
            _cell_w   = QWidget(tbl)
            _cell_w.hide()  # [FIX] setCellWidget 전 독립창 방지  # [FIX] parent 지정 — 재연결 시 floating 방지
...
# ─────────────────────────────────────────────────────────────
    ...
    def update_position_prices(self, oid: int, current_price: float):
        ...
        def set_close_position_callback(self, cb):
            self._close_pos_callback = cb

        def set_cancel_order_callback(self, cb):
            self._cancel_order_callback = cb

        def set_manual_modify_callback(self, cb):
            self._manual_modify_callback = cb

        def set_chaser_mode_callback(self, cb):
            self._chaser_mode_callback = cb

        def set_margin_mode_callback(self, cb):
            self._margin_mode_callback = cb
